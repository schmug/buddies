...
/**
 * Agent tooltip + mini thread popover, independent of how the agent was hit.
 *
 * Hovering shows true state (+ the real last message when sources are provided).
 * Opening a chat-slot agent shows recent messages, a composer that steers a
 * running turn or sends to an idle one, approve/deny for a blocked tool call,
 * and an "Open chat" action. The caller decides what counts as a hover or a
 * click — a canvas scene hit-tests pixels, a DOM scene uses element events —
 * and reports the anchor position in container-relative pixels.
 *
 * @param sources - live AgentSource list (enables last-message, approvals, and
 *                  the in-world "New session" button)
 * @param theme - scene-specific tooltip flavour line; omitted, the line is dropped
...
export function useAgentPopover(
  ...
  const close = useCallback(() => setThreadView(null), [])
  ...
  const open = useCallback((agent: SceneAgent, at: { x: number; y: number }) => {
    ...
    // Toggle off when opening the same agent
    if (threadView && threadView.agent.id === agent.id) {
      setThreadView(null)
      return
    }
    setTooltip(null)
    ...
    api.chatSlotDetail(slotKey, THREAD_VIEW_MESSAGES * 3)
      ...
      .catch(() => {
        setThreadView(tv => tv && tv.agent.id === agent.id ? { ...tv, loading: false, error: true } : tv)
      })
  }, [threadView])
  ...
  return {
    hover, clearHover, open, close,
    openAgentId: threadView?.agent.id ?? null,
    /** Currently hovered agent — the caller uses its kind to pick a cursor. */
    hoverAgent: tooltip?.agent ?? null,
    anchorRef,
